...
def process_hour(f_hour):
    ...
    # CRITICAL: Define time_label immediately to prevent NameError
    ...
    print(f">>> STARTING HOUR F{f_str} (Valid: {time_label}) <<<")
    try:
    ...
    # DATA EXTRACTION
    try:
        # Get SBCAPE (Index 4 from your log) and lat/lons
        cape_msg = grbs.select(shortName='cape', level=0)[0]
        cape = cape_msg.values
        lats, lons = cape_msg.latlons()
        
        # Force Surface Ground Level (Diagnostic Index 43)
    ...
    # SBCAPE Colorbar (Reverted to Magma as requested)
    ...
    for i in range(0, lats.shape[0], skip):
        for j in range(0, lats.shape[1], skip):
            ...
            try:
                ...
                # Plot color-coded AGL segments (Height minus Surface GH)
