...
def process_hour(f_hour):
    ...
    print(f">>> STARTING HOUR F{f_str} <<<") # Log check
    try:
    ...
    try:
        ...
        h_sfc = grbs.select(shortName='gh', level=0)[0].values
    except: return

    # Available levels from your diagnostic log
    avail_levels = [925, 850, 700, 500, 250]
    u_list, v_list, h_list, p_list = [], [], [], []
    ...
    for lev in avail_levels:
        ...
        p_list.append(np.full(u_list[0].shape, lev))
    ...
    # Original Magma
    clevs = [100, 500, 1000, 1500, 2000, 3000, 4000, 5000]
    ...
    for i in range(0, lats.shape[0], skip):
        for j in range(0, lats.shape[1], skip):
            ...
            try:
                ...
                # AGL Heights
                z_agl = h_stack[:, i, j] - (h_sfc[i, j] * units('m'))
                u_pts = u_stack[:, i, j].to('kt')
                v_pts = v_stack[:, i, j].to('kt')

                # PLOT: Using colormapped line
                hodo.plot_colormapped(u_pts, v_pts, z_agl,
    ...
    print(f"DONE: hodo_f{f_str}.png saved.")
