...
def process_hour(f_hour):
    ...
    print(f"Testing F{f_str} (Valid: {time_label})...")
    try:
    ...
    # Bolder Borders
    ax.add_feature(cfeature.STATES, edgecolor='black', linewidth=2.5, zorder=5)
    ax.add_feature(USCOUNTIES.with_scale('500k'), edgecolor='black', linewidth=0.8, alpha=0.7, zorder=5)
    ...
    for i in range(0, lats.shape[0], skip):
        for j in range(0, lats.shape[1], skip):
            ...
            try:
                ...
                h.add_grid(increment=20, color='gray', alpha=0.6, linewidth=0.8)
                
                h.plot_colormapped(u_stack[:,i,j].to('kt'), v_stack[:,i,j].to('kt'), h_stack[:,i,j] - h_stack[0,i,j],
                                     intervals=[0, 500, 3000, 6000, 9000] * units.m,
                                     colors=['#ff00ff', '#ff0000', '#00ff00', '#ffff00'], linewidth=2.0)
                
                ax_ins.plot(rm[0].to('kt'), rm[1].to('kt'), marker='o', color='red', markersize=2.0, zorder=10)
                ax_ins.plot(lm[0].to('kt'), lm[1].to('kt'), marker='o', color='blue', markersize=2.0, zorder=10)
                ax_ins.axis('off')
            except: continue
