...
# Setting the cycle to 00Z or 12Z based on current time
...
def process_hour(f_hour):
    ...
    # 1. Grab CAPE for Masking and Background
    try:
    ...
    cf = ax.contourf(lons, lats, cape, levels=cape_levels, colors=cape_colors, alpha=0.4)
    plt.colorbar(cf, orientation='horizontal', pad=0.03, aspect=50, label='SBCAPE (J/kg)')
    
    ax.add_feature(USCOUNTIES.with_scale('500k'), edgecolor='black', linewidth=0.2, alpha=0.2)

    # 3. Grid Loop with SPC Masking
    skip = 45 
    for i in range(0, lats.shape[0], skip):
        for j in range(0, lats.shape[1], skip):
            # MASK: Plot if CAPE >= 100 and within geographic extent
    ...
    plt.title(f"F{f_str} | Init: {DATE_STR} {CYCLE}Z", loc='right', fontsize=10)
